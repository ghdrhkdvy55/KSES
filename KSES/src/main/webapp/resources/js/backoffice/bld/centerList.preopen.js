...
$.Preopen.prototype.mainGridSettings = function() {
    ...
    $('#rightAreaUqBtn').html(
        '<a href="javascript:fnCenterPreopenPopup();" class="orangeBtn">복사</a>'
    ).show();
    $('#rightAreaBtn').show();
};
...
$.Preopen.prototype.preopenGridSetting = function() {
	// 직원 검색 JqGrid 정의
	EgovJqGridApi.pagingGrid('popGridPreopenCopy', [
        { label: '사전예약입장시간코드', name: 'optm_cd', key: true, hidden:true },
        { label: '요일', name: 'open_day_text', align: 'center', sortable: false },
        { label: '회원오픈시간', name: 'open_member_tm', align: 'center', sortable: false},
        { label: '회원종료시간', name: 'close_member_tm', align: 'center', sortable: false},
        { label: '비회원오픈시간', name: 'open_guest_tm', align: 'center', sortable: false},
        { label: '비회원종료시간', name: 'close_guest_tm', align: 'center', sortable: false},
    ], 'popPagerPreopenCopy', false, false, 10);
};
